  ...
  >
    ...
    >
      <div class="text-center mb-6">
        ...
        <!-- Titre de l'erreur -->
        <Title size="2xl" :glitch="true" className="mb-4 text-space-error">
          {{ title }}
        </Title>
        ...
        <div v-if="errorType === 'guild_required'" class="space-y-6 mb-6">
          ...
          <a
            ...
            class="inline-flex items-center justify-center rounded-lg transition-all duration-300 font-heading text-white border bg-space-primary border-space-primary hover:bg-space-primary-light hover:border-space-primary-light px-6 py-3 text-base mb-4 button-animate"
...
// Ouvrir le lien d'invitation Discord
const openDiscordInvite = (event) => {
  // S'assurer que le lien est valide
  if (!inviteLink.value || !inviteLink.value.includes("discord")) {
    event.preventDefault();
    console.error("Lien Discord invalide:", inviteLink.value);
    // Utiliser le lien par défaut si le lien est invalide
    window.open(
      "https://discord.gg/ksCGJztmBd",
      "_blank",
      "noopener,noreferrer"
    );
  }
  // Sinon, le lien normal fonctionnera
};
...
onMounted(() => {
  ...
  if (route.query.inviteLink) {
    const decodedLink = decodeURIComponent(route.query.inviteLink);
    // Vérifier que le lien est bien un lien Discord
    if (
      decodedLink.includes("discord.gg") ||
      decodedLink.includes("discord.com/invite")
    ) {
      inviteLink.value = decodedLink;
    }
  }

  // S'assurer que nous avons toujours un lien valide
  if (!inviteLink.value || !inviteLink.value.includes("discord")) {
    inviteLink.value = "https://discord.gg/ksCGJztmBd";
  }

  console.log("Discord invite link:", inviteLink.value);
