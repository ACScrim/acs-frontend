<template>
  <div class="relative notification-center">
    ...
    <button
      @click="togglePanel"
      class="relative p-2 text-normal-text hover:text-color-primary transition-colors rounded-lg"
        ...
        >
          ...
          <div class="p-3 sm:p-4 border-b border-color-primary/20">
            <div class="flex items-center justify-between">
              <h3
                class="text-normal-text font-nasa text-base sm:text-lg font-bold truncate mr-2"
              >
                Notifications
              </h3>
              ...
              >
                ...
                <button
                  v-if="unreadCount > 0"
                  @click="markAllAsRead"
                  class="text-normal-text-muted hover:text-normal-text text-xs transition-colors whitespace-nowrap hidden sm:block"
                ...
                <button
                  v-if="unreadCount > 0"
                  @click="markAllAsRead"
                  class="text-normal-text-muted hover:text-normal-text transition-colors sm:hidden"
                  title="Marquer toutes comme lues"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <button
                  @click="closePanel"
                  class="text-normal-text-muted hover:text-normal-text transition-colors"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-4 w-4"
          ...
          <div class="max-h-80 sm:max-h-96 overflow-y-auto">
            ...
            >
              <div class="text-normal-text-muted text-sm">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
            ...
            >
              <div
                class="p-3 sm:p-4 hover:bg-background-bg-light/50 transition-colors cursor-pointer"
              ...
              >
                <div class="flex items-start space-x-2 sm:space-x-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <h4
                      class="text-sm font-semibold text-normal-text font-nasa leading-tight"
                      :class="{ 'text-color-primary': !notification.read }"
                    >
                      {{ notification.title }}
                    </h4>
                    <p
                      class="text-xs text-normal-text-muted mt-1 line-clamp-2 sm:line-clamp-3 leading-relaxed"
                    >
                      {{ notification.body }}
                    </p>
                    <div class="flex items-center justify-between mt-2">
                      <span
                        class="text-xs text-normal-text-muted truncate mr-2"
                      >
                        {{ formatTime(notification.timestamp) }}
                      </span>
                      <button
                        @click.stop="deleteNotification(notification.id)"
                        class="text-normal-text-muted hover:text-color-error text-xs transition-colors flex-shrink-0 p-1"
                        title="Supprimer"
                      >
                        ✕
                      </button>
          ...
          >
            <button
              ...
              class="text-normal-text-muted hover:text-color-error text-xs transition-colors"
            >
