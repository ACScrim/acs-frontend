...
const addClip = async () => {
  try {
    ...
    if (!props.tournament._id)
      throw new Error(
        "Un problème est survenu lors de la récupération du tournoi"
      );
    if (!clipUrl.value.length) throw new Error("L'url du clip est manquante !");
    await tournamentService.addClipToTournament(props.tournament._id, {
      clipUrl: clipUrl.value,
    });
    ...
    const message =
      err instanceof Error ? err.message : "Erreur lors de l'envoi du clip";
...
const deleteclip = async () => {
  try {
    ...
    if (!clipToDelete.value?._id)
      throw new Error(
        "Un problème est survenu lors de la récupération du clip à supprimer"
      );
    await tournamentService.deleteClipFromTournament(
      props.tournament._id,
      clipToDelete.value._id
    );
    ...
    const message =
      err instanceof Error
        ? err.message
        : "Erreur lors de la suppression du clip";
    errMessage.value = message;
    showMessage("error", message);
  } finally {
    isLoading.value = false;
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 gap-y-8 pt-8">
      <Card v-for="(clip, idx) in tournament.clips" class="flex-1/2">
        <iframe
          class="w-full h-80"
          :src="clip.url"
          :title="`Clip ${idx + 1} ajouté par ${
            clip.addedBy.username
          } pour le tournoi ${tournament.name}`"
          frameborder="0"
          allow="clipboard-write; encrypted-media; picture-in-picture"
          allowfullscreen
        ></iframe>
        <div
          class="flex flex-row justify-between items-center text-color-text-muted"
        >
          <p>
    ...
    >
      <div class="space-y-3 sm:space-y-4">
        ...
        <p class="text-color-text-muted text-sm sm:text-base">
          Vous devez insérer un lien
          <b class="text-color-primary-light">youtube ou twitch</b> valide dans
          <b class="text-color-primary-light">l'un des formats suivants</b> :
        </p>
        <ul class="list-disc list-inside text-color-text-muted">
        ...
        <div
          class="border-l-4 border-red-400 p-4 rounded-md flex items-start gap-3"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
          ...
          <div>
            ...
            <ul
              class="text-red-400 text-sm sm:text-base list-disc pl-5 space-y-1"
            >
              <li>
                Votre clip sera <span class="font-bold">refusé</span> si le
                format n'est pas respecté.
              </li>
              <li>
                Les administrateurs se réservent le droit de
                <span class="font-bold">supprimer</span> tout clip ne
                correspondant pas au tournoi.
              </li>
            </ul>
          </div>
        </div>
